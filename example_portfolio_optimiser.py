...
techniques to calculate the optimal portfolio by maximising the sharpe ratio.
...
class portfolio_opmitiser:
    ...
    def get_price_matrix(self):
        return self.analyze.price_matrix(self.ticker_list,
                                         self.start_date, 
                                         self.end_date)
...
print(f"stocks:          {random_tickerlist}")
...
optimal_weights = optimise.get_optimal_weights()
print(f"optimal weights: {[str(f'{x:.2f}%') for x in optimal_weights]}")
optimise.plot_efficient_frontier()
optimise.plot_indexed_return_chart()

# manual usage example: 
# optimise = portfolio_opmitiser(['SLB','LULU','HES','DAL'], '2022-10-21', '2023-10-20')
